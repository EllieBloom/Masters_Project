...
plot_workplace_lockdown1 <- ggplot(data=google_england_lockdown1, aes(x=date,y=workplaces)) +
                                      ...
                                      xlab("") +
                                      ylab("Workplace mobility compared to baseline (%)") +
                                      ylim(-90,-10) +
                                      ggtitle("A") +
...
plot_grid(plot_workplace_lockdown1,
          plot_workplace_lockdown2,
          plot_workplace_lockdown3,
          nrow=1, align="h")
...
ggsave(file="line_model_fit_workplace.png", plot_grid(plot_workplace_lockdown1,
                                                      plot_workplace_lockdown2,
                                                      plot_workplace_lockdown3,
                                                      nrow=1, align="h"), width=365, height=140, units="mm") 
...
for (i in 1:length(mobility_types)){
  ...
  data$days_since_lockdown <- as.numeric(data$date - start_date)
  models <- lmList(eval(parse(text=mobility_types[i])) ~ day + bank_holiday + days_since_lockdown | sub_region_1, data=data, na.action=na.exclude)
...
# Extracting CIs for highest and lowest counties
...
plot_workplace_whole <- ggplot(data=google_whole_model, aes(x=date,y=workplaces_av)) +
  ...
  scale_y_continuous(limits=c(-90,-10),expand=c(0,0))+
  labs(y="Workplace mobility compared to baseline (%)",
       x="") +
  #ylim(-90,-10) +
  theme_bw() +
...
plot_workplace_whole_additions <- plot_workplace_whole + annotate("rect", xmin=lockdown_1_start, xmax = lockdown_1_end, ymin=-90, ymax=-10, alpha=0.2) +
                                                          annotate("rect", xmin=lockdown_2_start, xmax = lockdown_2_end, ymin=-90, ymax=-10, alpha=0.2) +
                                                          annotate("rect", xmin=lockdown_3_start, xmax = lockdown_3_end, ymin=-90, ymax=-10, alpha=0.2) 


plot_workplace_whole_additions
setwd("~/Desktop/Masters/Project/Analysis/NPIs_mobility_updated/Outputs/Regression/Plots")
ggsave("line_model_fit_whole_period_workplace.png", plot_workplace_whole_additions, width=365, height=200, units="mm")
